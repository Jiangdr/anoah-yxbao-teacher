...
_html2canvas.Preload = function( options ) {
  ...
  doc = element.ownerDocument,
  domImages = $("img" , element).get(), // TODO probably should limit it to images present in the element only
  ...
  function proxyGetImage(url, img, imageObj){
    ...
    script;
    if(_html2canvas.logging)console.log('use proxy');
    link.href = url;
...
_html2canvas.Renderer.Canvas = function(options) {
  ...
  return function(zStack, options, doc, queue, _html2canvas) {
    ...
    var bounds = _html2canvas.Util.Bounds(options.elements[0]);


	if(bounds.height > canvas.height)canvas.height = bounds.height;
	if(bounds.width > canvas.width)canvas.width = bounds.width;
    ...
    if (queueLen === 1) {
      if (typeof options.elements[0] === "object" && options.elements[0].nodeName !== "BODY") {
        ...
        ctx.drawImage(canvas, bounds.left, bounds.top, parseInt(bounds.width), parseInt(bounds.height), 0, 0, bounds.width, bounds.height);
        canvas = null;
        return newCanvas;
      }
    }
