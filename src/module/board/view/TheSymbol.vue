    ...
    <ul class="fl num-ul">
      <li
        ...
        class="clear-fix k-row k-row-4">
          <span
            class="fl k-btn k-btn-6"
            :class="{['k-' + cp_ordinary[idx * 6 + i].type]: 1}"
            v-for="(btn, i) in 6"
            v-if="(idx * 6 + i) < cp_ordinary.length"
            @click.stop="$emit('btn-click', $event, cp_ordinary[idx * 6 + i])"
            :key="i">{{cp_ordinary[idx * 6 + i].view || cp_ordinary[idx * 6 + i].text | CharCodeIcon}}</span>
      </li>
    </ul>
    <ul class="fr sym-ul sym-pad">
      <li
        ...
        <span
          ...
          :key="i">{{btn.view || btn.text | CharCodeIcon}}</span>
      </li>
    </ul>
  </div>
</template>
