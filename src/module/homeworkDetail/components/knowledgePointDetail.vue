<template>
  <div id="knowledge-point-detail">
    <header-bar @back="goBack">
      <div slot="title-name">知识点</div>
    </header-bar>
    <div class="content">
      <div class="percent-wrapper van-hairline--bottom">
        <div class="percent-item">
          <span class="name">正确率</span>
          <span class="percent red">{{Math.round($route.params.correct_rate * 100) + '%'}}</span>
        </div>
        <div class="percent-item">
          <span class="name">题数</span>
          <span class="percent black">{{$route.params.qti_count}}</span>
        </div>
      </div>
      <div class="list">
