      ...
      <div v-for="(item,key) in record" :key="key" v-if="item.students">
        <van-row class="item">
          ...
          <van-col span="20" class="right">
            <span class="column"  :style="{width:(item.count/count)*100+'%'}" :class="{'right-answer':key=='right','no-answer':key=='no'}" @click='item.students.length>0?toggleAllCorrec(itemName(key)+"的学生",item.students):""'></span>
