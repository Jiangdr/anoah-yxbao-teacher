    ...
    <div class="order-question">
      ...
      <div class="container">
        <div  v-for="(item,index) in resource" :key="index">
          ...
          <div v-if="item.child" class="item" @click="detail(index)">
              <van-row v-if="item.child.length" v-for="(kp,i) in item.child" :key="i">
                <van-col span="8" offset="8">{{kp.content}}</van-col>
                <van-col span="8">
                  <p>
                    <span>{{itemCorrect(kp.correct_rate)}}</span>
                    <span class="progress" v-if="kp.correct_rate!=''||kp.correct_rate!=-1">
                        <span
                          class="progress-bar"
...
export default {
  ...
  methods: {
    ...
    detail(index) {
      ...
      this.$router.push({name: "groupDetailColumn", params: {'data': this.resource, 'index': index}})
    }
  }
}
</script>
